...
ddtheta = T_EXT; % external torque given as input
dtheta_shoulder = qdot(1,:);
dtheta_elbow = qdot(2,:);
%think the a variables refer to the muscle activations
a1 = auxdata.I1 + auxdata.I2 + auxdata.m2*auxdata.l1^2;
a2 = auxdata.m2*auxdata.l1*auxdata.lc2;
a3 = auxdata.I2;

for i = 1:size(T,2)
    %Mass and Coriolis force matrices
    ...
    ddtheta(:,i) = M\(T(:,i) + T_EXT(:,i) - C - B*qdot(:,i));%Think this is equation 21 from the paper
end
end
