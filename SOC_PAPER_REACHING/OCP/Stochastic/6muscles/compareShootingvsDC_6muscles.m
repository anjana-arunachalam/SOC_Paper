...
for i = 1:length(wM_std_VEC)
    for j = 1:length(wPq_std_VEC)
        for k = 1:length(wPqdot_std_VEC)
            ...
            %What is the difference between the saveName and guess Name
            %variable definitions? 
            
            %Results for the circle target simulation from the Multiple
            %shooting and the direct collocation methods
...
%Wonder why all the target guesses are circles?
%And the comparison is only for an unperturbed system